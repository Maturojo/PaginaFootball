      ...
      <div className="max-w-6xl mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div>
          ...
          <ul className="space-y-1 text-sm text-white/60">
            {['/', '/historia', '/equipos', '/estadisticas', '/eventos', '/tienda', '/testimonios', '/contacto'].map((to, i) => (
              <li key={to}><Link to={to} className="hover:text-accent transition">{['Inicio','Historia','Equipos','Estadísticas','Eventos','Tienda','Testimonios','Contacto'][i]}</Link></li>
            ))}
          </ul>
        </div>
        <div>
